...
function setValue(dictionary, key, value) {
  ...
  if (dictionary.has(key)) {
    ...
    return dictionary.set(value, key);
  }
}
...
function transformDictionary(dictionary) {
  ...
  const obj = keys.concat(values, entries);
...
function getFilteredDictionarySize(dictionary, filter) {
  ...
  for (const entry of dictionary.values()) {
    entry > 1000000 ? map.set(entry) : false;
  }
  return map.size;
}
